...
void DepthChessboardPoseEstimator::image_callback(const sensor_msgs::Image::ConstPtr& image_messages)
{
  cv::Mat frame(
...
void DepthChessboardPoseEstimator::depth_callback(const sensor_msgs::PointCloud2::ConstPtr& depth_messages)
{
  ...
  if (!(std::isnan(x) + std::isnan(y) + std::isnan(z)))
  {
    ...
    tf_broadcaster.sendTransform(tf::StampedTransform(tf_transform, ros::Time::now(), "base_link", "depth_chessboard_tf"));
  }

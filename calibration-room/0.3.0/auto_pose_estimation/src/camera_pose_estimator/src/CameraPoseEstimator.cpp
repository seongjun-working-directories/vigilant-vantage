...
{
  image_subscriber = image_transport::ImageTransport(node_handle).subscribe("/usb_cam/image_raw", 1, &CameraPoseEstimator::camera_callback, this);
...
void CameraPoseEstimator::camera_callback(const sensor_msgs::Image::ConstPtr& image_messages)
{
  // [TEST] std::cout << (image_messages->data).size() << std::endl;

  cv_bridge::CvImageConstPtr cv_ptr;
  cv_ptr = cv_bridge::toCvCopy(image_messages, sensor_msgs::image_encodings::BGR8);
  cv::Mat input_frame = cv_ptr->image.clone();
